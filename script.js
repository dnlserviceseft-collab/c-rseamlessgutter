...
(function(){
  ...
  const openLB = (clicked) => {
    ...
    lbList = visible.map(itemData);
    lbIndex = Math.max(0, visible.indexOf(clicked));
    renderLB(); lb.classList.add('open'); document.body.style.overflow='hidden';
  };
  const closeLB = () => { lb.classList.remove('open'); document.body.style.overflow=''; };
  const moveLB = (dir) => { if(!lbList.length) return; lbIndex = (lbIndex + dir + lbList.length) % lbList.length; renderLB(); };
  allItems.forEach(it => it.addEventListener('click', () => openLB(it)));

  /* ---------- Gallery filter ---------- */
  const gfBtns = [...document.querySelectorAll('.gf-btn')];
  gfBtns.forEach(b => b.addEventListener('click', () => {
    gfBtns.forEach(x => x.classList.remove('active'));
